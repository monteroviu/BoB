class InvitationsController < ApplicationController
  ...
  def create
    ...
    puts " >>>>>>>>>>>>>>>>>>>>>>>>>>>parametros>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
    puts invitation_params.inspect
    puts " >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
  ...
  #   @student=Group.find(params[:student_id])
  #   @invitation (1, @group.id, @student.id)
  # end
  ...
  private
    ...
    def invitation_params
      params.require(:invitation).permit(:questionnaire_id, :group_id, :student_id)
    end
end
